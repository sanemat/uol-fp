## Chapter 3

The research process.
The components of the research process.
some models.

Six research strategies for computing: 
survey, design and creation, experiment, case study, action research and ethnography.

Survey (Chapter 7)
Design and creation (Chapter 8)
Experiment (Chapter 9)
Case study (Chapter 10)
Action research (Chapter 11)
Ethnography (Chapter 12)

"Typically one research question has one research strategy."

Four data generation methods:
Interview (Chapter 13)
Observations (Chapter 14)
Questionnaire (Chapter 15)
Documents (Chapter 16)

"Some data generation methods are commonly associated with particular research strategies."

"one research strategy can use more than one data generation method."

"method triangulation"
"The use of more than one data generation method to corroborate findings and enhance their validity."

1. What is it about?

The 6Ps of research:
- The purpose of research (the reasons people do research)
- The products of research (outcomes from research project)
- The process of research (the sequence of activities you need to perform)
- The participants of research (who or what is involved?)
- The paradigm of research (what view of knowledge guides the research?)
- The presentation of research (How is the research shown or reported?)

2. What part of my project does it support?

Canonical textbook definitions of six core research designs for computing and IS research.
Chapter 7 (Survey)
Chapter 8 (Design and creation)
Chapter 9 (Experiment)
Chapter 10 (Case study)
Chapter 11 (Action research)
Chapter 12 (Ethnography)

Four data generation methods:
Chapter 13 (Interview)
Chapter 14 (Observations)
Chapter 15 (Questionnaire)
Chapter 16 (Documents)

3. What does it not solve?

Automated or computational extraction of methodology.

4. How will I use it?

Oates provides background on research methodology in computing and information systems.

5. Useful terms / concepts:
6. One or two key quotes or page references:

- How does Oates explain research methodology?
- What are the main research strategies or methods in computing / IS?
- How is methodology different from a single method?
- How is methodology connected to the research process?
- Which ideas can support my structure: Design / Method / Data / Evaluation?
- What does Oates not solve for my project?
