...
For example, a machine learning paper may introduce a new model (TechnicalMethod), apply it to a task such as machine translation (Task), use a public dataset (Data), and evaluate the result with a metric such as BLEU (Evaluation). The paper's overall research strategy — proposing a new artefact and benchmarking it — is its ResearchDesign. A human reader can usually understand this structure from the paper text. This project aims to extract it automatically and represent it as a structured profile. Existing computational systems can extract some parts of this profile — model names, tasks, datasets, or metrics — but do not reliably identify the paper's overall ResearchDesign (see Section 3). This is the gap this project addresses.
