# Prototype: Document-Level Methodology Extraction (869 words, exclude: References, Appendix, figures, tables)
...
Code quality: `pyright` (`strict` mode) and `ruff` report zero issues; `proto3/src/uol_fp/scoring.py` now has 14 pytest tests, though the `RoleExtraction` null-correlation check still has none.
