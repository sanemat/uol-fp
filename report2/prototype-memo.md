...
A: I use `pyright` in `strict` type-checking mode and `ruff` for linting (`select = ["E", "F", "I"]`) and formatting — the same strict setup as proto2, now applied to this new pipeline. The notebook is organized into named, ordered stages (Setup → Data Models → Stage 0 → Stage 1 → Stage 2 → Stage 2b → Stage 2c) as markdown headers, so the pipeline structure is visible directly in the table of contents. One honest limitation remains: this is still notebook code mixing exploratory output with pipeline logic. JSON parsing itself no longer needs a test for correctness — `response_json_schema` forces Gemini's reply to already be valid JSON in the required shape, and `MethodologyProfile.model_validate_json(...)` just delegates to Pydantic, so there is no custom parsing code of mine to test. The one piece of custom validation logic in that model, `RoleExtraction`'s `model_validator` that checks `answer` and `evidence` are both `null` or both present (Q8), is not guaranteed by `response_json_schema` the way the JSON shape itself is — the schema constrains each field's type, not this cross-field correlation — so I added a direct test for it (below). A second issue I found and fixed: `proto3/baseline.ipynb` used to be a byte-identical duplicate of `proto3/3pipeline.ipynb`, kept only to have produced the six `proto3/baseline/*.json` outputs. I deleted it — `proto3/baseline/*.json` is now the sole frozen artifact, and `proto3/update_baseline.py` regenerates the notebook's copy of those answers directly from the JSON files, so there is no second notebook to keep in sync by hand.

I also moved two things out of the notebook into proper installable modules under `proto3/src/uol_fp/`, each with its own `pytest` tests: the Stage 3 evaluation logic (`normalize`, `matches`, `score_role`, `score_profile`, `precision_recall_f1`) into `scoring.py`, and the `Evidence`/`RoleExtraction`/`MethodologyProfile` Pydantic models (Q8) into `models.py`. `proto3/tests/test_scoring.py` covers `scoring.py` — the `None`-handling and substring-match rules in `matches`/`normalize`, all four TP/FP/FN/TN branches of `score_role`, and the precision/recall/F1 edge cases (zero-division) — and `proto3/tests/test_models.py` directly exercises `RoleExtraction`'s null-correlation `model_validator`: both fields `None`, both present, and the two mismatched cases (`answer` set with `evidence` `None`, and the reverse), asserting the mismatched cases raise `pydantic.ValidationError`. One script, `proto3/sync_generated.py`, regenerates both notebook cells (the Data Models cell from `models.py`, the scoring-functions cell from `scoring.py`) from these two source files — the same pattern already used for `BASELINE_ANSWERS`, so the tested code and the notebook code cannot drift apart; this replaced an earlier version that only handled `scoring.py`, since adding a second module made it clear the notebook-sync logic itself should not be duplicated per module. Getting `scoring.py` to pass `pyright --strict` needed two real fixes, not just suppressions: `score_role`'s original `if X is None and Y is not None` conditions could not be narrowed by the type checker, so I rewrote them as nested `if X is None: ...` checks; and `matches` needed an explicit `assert g is not None and s is not None` to state the invariant that `normalize()` only returns `None` for a `None` input, which pyright cannot infer from the function signature alone. `ruff check`/`ruff format` are also now clean across `proto3/src/`, `proto3/tests/`, `3pipeline.ipynb`, and the `proto3/sync_generated.py`/`proto3/update_baseline.py` generator scripts — mostly line-length fixes, plus making `update_baseline.py`'s generator wrap an overlong baseline value (e.g. MapReduce's `Task` sentence) in parentheses across two lines rather than suppressing the check, since the line length there is data-driven, not a style choice. Note `pyright` is only run against the `.py` files, not `3pipeline.ipynb` directly — the plain CLI `pyright` (unlike the Pylance/VS Code extension) does not parse `.ipynb`'s JSON structure and instead misreads raw JSON tokens like `"execution_count": null` as broken Python, so it has no real target there; `ruff` is the notebook's actual lint/format tool, via its `extend-include = ["*.ipynb"]` config.
